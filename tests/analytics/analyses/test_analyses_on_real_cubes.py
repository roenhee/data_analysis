...
def test_the_known_version_comparison_is_nearly_cancelled_by_composition(real_cubes):
    ...
    **계획서가 적어 둔 "합산 음수" 는 재현되지 않았다.** 측정값(22일치, 배포일 컷오프
    이후 10일)은 날짜별 +4.4~9.2% 인데 합산은 +0.017 걸음(≈+0.2%)에 그친다 — 부호가
    뒤집히는 게 아니라 구성 변화가 효과의 대부분을 먹는다(within +0.076, between -0.058).
    교훈은 같다: 합산 숫자만 보면 "효과 없음" 이라고 결론낸다. 부호까지 뒤집히는 조합은
    이 22일치에도 없다.
    ...
    assert 0 < pooled < per_day.min(), "합산이 어느 하루치 효과보다도 작아야 한다"
...
def test_gender_comparison_is_stable_across_days(real_cubes):
    """실측: F vs M 기대 걸음 수는 22일 내내 부호가 안 바뀌고 좁은 띠에 있다.

    계획서는 -11.1%~-6.6% 로 적었지만 실측은 날짜별 -0.19~-0.26 걸음의 좁은 띠였다
    (측정 범위가 다르다). 고정할 값은 크기가 아니라 **성질**이다 — 날짜 가중치가 거의
    안 어긋나서(skew 0.008) 합산이 날짜별 평균과 같은 자리에 있다. 버전 비교와 정반대
    상황이라 대조군이 된다.
    """
    got = compare(real_cubes, "screen_flow", on="gender", a="F", b="M")
    assert len(got.dates_used) == 22
    per_day = got.per_day["delta_mean_expected_steps"]
...
def test_quality_warnings_are_backed_by_volume_not_the_long_tail(real_results):
    ...
    건짜리 버전의 100% 였다. 접은 뒤에는 22일·7서비스에 113건(봉투 17KB)이고 전부
    물량이 뒷받침한다 — 롱테일 폭발(수만 건)과는 자릿수가 다르다.
    """
    import json
    ...
    assert len(warnings) < 200, f"{len(warnings)}건 — 집계 수준이 잘못됐다"
...
def test_the_pooled_flow_headline_sits_outside_every_service(real_cubes):
    ...
    실측: 합산 **10.7** 대 서비스별 2.62(content_v)~8.05(top). 이탈확률은 반대로 합산
    **0.0967** 이 최소 0.1411 보다 **낮다** — 벗어나는 방향이 위쪽만이 아니다. 화면 간
    전이의 50.2%가 서비스를 건너뛰어서, 합친 체인에는 어떤 단일 서비스 안에도 없는
    ...
    assert got.services == ["agorax", "content_v", "entertain", "media", "search",
                            "sports", "top"]
...
def test_the_dictionary_starves_the_small_services(real_results):
    ...
    먼저 잘린다. 전체로는 4.89%인데 서비스별로는 agorax **100%** · sports **37.49%** ·
    entertain **19.30%** · top 3.06% · media 0.51% · content_v 0.003% · search 0% 다.
    가장 작은 agorax(물량 0.14%)는 상위 화면에 하나도 못 들어 전량이 `/other` 다 —
    사전이 작은 서비스를 굶긴다는 게 극단적으로 드러난 경우다.

    `/other` 는 드문 화면이 아니라 여러 이름을 접은 가짜 화면이다. 상태를 합치는 것은
    합쳐진 화면들의 나가는 분포가 같을 때만 무손실이므로, sports·agorax 의 기대 화면 수는
    상태 하나가 그 버킷인 체인의 값이다. **전체 한 숫자로는 그게 안 보인다.**
    """
    got = real_results["screen_flow"]
    shares = got.envelope["other_share"]
    assert shares["agorax"] == pytest.approx(1.0), "가장 작은 서비스는 전량이 /other 다"
...
def test_the_service_mix_shows_the_pooled_number_is_mostly_top(real_results):
    """실측 top 61.5% 대 content_v 2.2%·agorax 0.14%. 봉투에 없으면 합산이 "앱 전체" 로 읽힌다."""
...
def test_cross_service_movement_is_about_half_of_screen_transitions(real_results):
    ...
    실측 `cross_service_share` 0.5016, `switch_entropy` 2.2483 nats. 건너뛰는 쌍이
    수십 개라 최대 엔트로피의 60%대 — 한 경로로 몰리는 게 아니라 여러 방향으로 흩어진다.
    """
...
def test_the_session_cube_version_comparison_moves_opposite_to_the_flow_one(real_cubes):
    ...
    **내려갔다.** 세션당 체류는 합산 −0.10초, 날짜별 −0.44~−0.05초로 10일 다 음수다.
    같은 두 버전의 기대 걸음 수는 +4~9% 였다(위 `screen_flow` 테스트) — **두 지표가
    반대로 움직인다.** 9.5.1 세션은 화면을 더 많이 밟으면서 더 짧게 머문다.

    다만 이 비교는 그대로 읽으면 안 된다: `weight_skew` 0.68 이고, `within` 은 거의
    전부 07-26 에서 온다 — 그날 9.5.0 이 9.5.1 의 44배라 배포일 컷오프를 지나고도
    램프업 첫날의 소수 집단을 재고 있다. 고정할 것은 크기가 아니라 **부호와 그 취약함**이다.
    """
    ma = real_cubes.filter(service_type="MA")
...
def test_the_busiest_pair_is_not_the_most_affine_one(real_results):
    ...
    카운트 1위는 `top/엠탑조회` 자기 루프인데 PMI 로는 285쌍 중 하위권이다. PMI 1위는
    관측 몇백 건짜리 자기 루프다 — 얇은 셀의 PMI 가 가장 크게 튄다는 경고가 `cnt` 열과
    함께 봉투에 있는 이유다.
    ...
    assert 275 <= got.headline["pairs"] <= 295
