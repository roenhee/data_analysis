...
### ③ 군집이 **다른 그래프**를 보고 있다 — **중간, 그리고 이름이 같아서 위험하다**
...
|---|---|---|
| 제외 상태 | `excl_lifecycle` — 생애주기 상태 전체 | `START`·`EXIT` 만 |
| resolution | **1.2** | 기본 **1.0** (파라미터로 바꿀 수 있다) |
| 그래프 원천 | **5-gram 상위 엣지** (`top5gram10`) | **1-step 전이** |

세 번째가 핵심이다. 5-gram 기반 그래프는 "한 경로 안에 함께 등장하는 경향" 이고 1-step 은
"직접 이어짐" 이다. **같은 이름의 분석이 서로 다른 대상을 군집화한다** — 노트북 결과와
현재 결과를 나란히 놓고 "군집이 달라졌다" 고 읽으면 틀린다.

`path` 큐브가 생겼으므로 5-gram 엣지는 이제 만들 수 있다. 하지만 어느 쪽이 맞는 질문인지가
먼저다 — 현재 문서가 "군집 수를 강한 사실로 읽지 말 것" 이라고 적어 둔 이유(화면 15개,
modularity 0.394, 노드 순서로 3개↔4개)는 그래프 원천과 무관하게 유효하다.
