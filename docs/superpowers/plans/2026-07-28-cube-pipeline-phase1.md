...
**✅ `dur_sum` 결함 — 확인되어 수정됨 (2026-07-29).** 초안은 `screens` 가
`action_type='Pageview'` 행만 읽고 그 행의 `usage.duration` 을 더했다. 하루치 빌드에서
`dur_sum` 이 **100% 0** 으로 나왔고, 원천을 직접 세어 원인을 확정했다
(`top`, 2026-07-27):

| action.type / kind | 행수 | `usage.duration` 비NULL |
|---|---|---|
| Pageview / (kind 없음) | 102,461,593 | **0** |
| Pageview / ViewContentList | 69,686,896 | **0** |
| Usage / UsagePage | 139,232,353 | 139,115,258 (99.9%) |

수정: `stream` CTE 가 두 신호를 합치고, `visit_idx`(그 행까지의 Pageview 수)로 각 Usage 행을
직전 화면 방문에 묶는다. 방문 하나가 여러 Usage 행을 낼 수 있어(`media` 는 107%) 방문 단위로
먼저 합친다. `cnt` 는 영향받지 않는다 — 상태를 만드는 것은 여전히 Pageview 행뿐이다.

**단위는 밀리초였다.** 첫 수정 후 평균 체류가 78,735"초"(21.9시간)로 나와 원본 분포를 쟀다:
중앙값 5,285 · p90 29,971 · **최대 10,799,978 = 정확히 3시간**(서버측 상한). 초로 보면 125일이라
성립하지 않는다. `_event_cte` 는 `usage_duration_ms` 로 이름에 단위를 박고, transition SQL이
1000으로 나눠 **초**로 낸다 — 세션 큐브의 `duration_sum`(`date_diff('second', ...)`)과 같은 단위.

**체류 커버리지는 축마다 다르다** (2026-07-27 실측, UsagePage/Pageview 행 비율):

| 축 | 커버리지 |
|---|---|
| `search` (전 OS) | **0%** — 체류 신호 자체가 없음 |
| `top` android / ios / 웹 | 84.8% / 85.8% / 65.6% |
| `media` / `entertain` | 105.1% / 103.0% (방문당 여러 건) |
| `top` 9.5.1 vs 9.5.0 | **88.7% vs 88.3%** |
| `top` 9.5.1 vs 7.4.1 | 88.7% vs 70.4% |

위 표는 **행 비율**이라 과대평가다. **방문 단위 실측**(`dur_n / cnt`)은 `top` 전체 56.8%,
화면별 57~69%. 판단에는 방문 단위를 쓴다. 수정 후 방문당 평균 체류는 `홈탭_진입` 24.9초 ·
`다음_PC탑` 64.2초 · `/other` 6.9초로 나왔다(두꺼운 셀 중앙값 25.0초).

따라서 **`dur_sum` 을 전수 절대값으로 쓰면 안 된다.** `dur_n`(체류가 측정된 방문 수)을 같이
내고 `dur_sum / dur_n` 으로 읽는다 — `dur_sum / cnt` 는 커버리지만큼 축소된 틀린 값이다.
`dur_n / cnt` 가 그 셀의 커버리지다. 인접 버전쌍(스펙의 실제 용도)은 편향이 0.4%p 로
상쇄되므로 델타가 유효하고, 서비스 간·OS 간·세대 간 절대 비교는 무효다.
**유효 조건은 "커버리지가 높다"가 아니라 "비교 대상 둘의 커버리지가 같다"** 이다.
quality 큐브의 `screen_without_dwell` 이 이걸 상시 감시한다.

**얇은 셀 주의.** 실측 엣지 셀의 `cnt` 중앙값은 9이고 18.9%는 1이다. 엣지 단위 평균 체류는
두꺼운 셀에서만 의미가 있다. `dur_sum`·`dur_n` 은 둘 다 가산이므로 `to_state` 로 합쳐
화면 단위(셀이 5.6배 두꺼움)로 올려서 본다.

**비용**: 체류 귀속이 윈도 함수에 들어가는 행 수를 약 2배로 늘린다(`top` 하루 기준
Pageview 1.7억 + UsagePage 1.4억). 하루·1서비스 빌드가 10분을 넘길 수 있다.
...
**실측 결과 (2026-07-29, 2026-07-27 하루, 6개 서비스)**: session 17,811행 / transition
**229,940행**(스펙 214,368 대비 +7%, 허용 구간 안) / quality 14,043행. state 사전은
screens 10 · layer1 42 · layer2 169 · versions 16. **`dur_sum` 은 100% 0으로 나왔다** —
아래 ⚠️ 대로 원천을 확인한 결과 `top` 하루치에서 Pageview 행의 `usage.duration` 비NULL이
**0건**이고, 체류시간은 전부 `Usage`/`UsagePage` 행(1억 3,923만 건 중 1억 3,912만, 99.9%)에
실려 있다. 스펙이 맞고 이 큐브의 `dur_sum` 은 구조적으로 죽은 측정값이다.
**후속 태스크로 분리한다** (Task 10의 ⚠️ 참조).
...
**실측 결과 (2026-07-29 실행, `weather` 2026-07-26~28)**: 큐브 936,385 / 단일 936,772 /
차이 **-387건(-0.041%)**. 부호가 **음수**인 것이 핵심이다 — 07-26 이전에 시작한 세션이
큐브에선 제외되고 단일 집계엔 잡히는 정상적인 경계 효과다. 중복 집계라면 큐브 쪽이
더 커야 한다. **통과.**
...
### 캐시 키의 SQL 로직 지문 (2026-07-29 발견 → 같은 날 수정됨)

초안의 캐시 키는 `(source_version, state_dict_version, axes, cube_name)` 이라 **집계 SQL을
고쳐도 키가 그대로였다.** `dur_sum` 을 고친 뒤 그냥 다시 빌드하니 옛 큐브가 캐시 적중으로
나왔고, 검증에 `refresh=True` 를 써야 했다. "조용히 틀린 숫자"의 전형 — 로직을 고쳤는데
결과가 안 바뀌고 그걸 눈치채지 못한다.

수정: 키에 `sql_hash` 를 추가했다. `builder._logic_hash` 가 **날짜만 고정한**(`2000-01-01`)
SQL을 만들어 해싱하므로 지문은 날짜 간에 같고(범위 읽기가 깨지지 않는다), 집계 로직·
서비스 목록·state 사전·테이블 좌표가 바뀌면 달라진다. 수동으로 올리는 버전 상수는
올리는 걸 잊으므로 쓰지 않는다.

부수 효과로 **서비스 목록 충돌도 막힌다.** `services` 는 키의 다른 어떤 항목에도 없어서,
서로 다른 서비스 묶음이 우연히 같은 state 사전을 내면 같은 경로에 겹쳐 쓰였다.
