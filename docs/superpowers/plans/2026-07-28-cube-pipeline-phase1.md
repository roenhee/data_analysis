...
### 알려진 갭 (2026-07-29 체류 수정 중 발견, 미해결)

**큐브 캐시 키에 SQL 로직 버전이 없다.** 키는
`(source_version, state_dict_version, axes, cube_name)` 인데, 집계 SQL 자체를 고쳐도
이 넷 중 아무것도 바뀌지 않는다. 그래서 `dur_sum` 을 고친 뒤 그냥 다시 빌드하면
**옛 큐브가 캐시 적중으로 그대로 나온다.** 검증할 때 `refresh=True` 를 써야 했다.

이건 "조용히 틀린 숫자"의 전형이다 — 로직을 고쳤는데 결과가 안 바뀌고, 안 바뀐 걸
눈치채지 못한다. 고치려면 생성된 SQL의 해시를 키에 넣는 게 가장 확실하다
(수동으로 올리는 버전 상수는 올리는 걸 잊는다). `store.cube_key` 시그니처와
`builder.build_cubes` 의 `key_parts`, 그리고 `test_store.py` 가 영향받는다.
